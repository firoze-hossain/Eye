...
public class InstallationService {
    ...
    // FIX: don't blow up when the server sends fields this DTO doesn't declare
    // (the server's RegistrationResponse includes organizationName, subdomain,
    // userEmail, etc.). Previously this threw UnrecognizedPropertyException.
    ...
    public RegistrationResult registerDevice(String email, String registrationToken) {
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                return RegistrationResult.failure("Server error: " + response.statusCode() + " - " + response.body());
            }
    ...
    @lombok.Data
    @JsonIgnoreProperties(ignoreUnknown = true)   // belt-and-suspenders
