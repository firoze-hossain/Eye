...
public class UserService {
    ...
    /**
     * Role-aware employee listing: admins see the whole organization; a
     * supervisor sees only the employees assigned to them (managerId == their id).
     */
    public List<UserResponse> getEmployeesScoped(Long organizationId, Long callerUserId, String callerRole) {
        if ("admin".equalsIgnoreCase(callerRole)) {
            return getEmployeesByOrganization(organizationId);
        }
        return userRepository.findByOrganizationIdAndManagerId(organizationId, callerUserId).stream()
            .map(this::toUserResponse)
            .collect(Collectors.toList());
    }

    /** Admin-only: assign (or clear, with managerId=null) an employee's manager. */
    /** True if the caller (admin, or a supervisor viewing their own team) may view targetUserId. */
    public boolean canView(Long organizationId, Long callerUserId, String callerRole, Long targetUserId) {
        if ("admin".equalsIgnoreCase(callerRole)) return true;
        if (callerUserId.equals(targetUserId)) return true; // can always view own record
    ...
    public void assignManager(Long organizationId, Long employeeId, Long managerId) {
        ...
        if (managerId != null) {
            ...
            if (!manager.getOrganizationId().equals(organizationId)) {
