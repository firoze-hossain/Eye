...
public class OrganizationService {
    ...
    private final OrganizationRepository organizationRepository;
    private final UserRepository userRepository;
    private final CryptoService cryptoService;      // still used for the device registration token
    private final PasswordService passwordService;  // NEW: proper password hashing
    ...
    public RegistrationResponse registerOrganization(PublicController.OrganizationRegistrationRequest request) {
        ...
        // FIXED: one-way BCrypt hash instead of reversible AES encryption.
        admin.setPasswordHash(passwordService.hash(request.getPassword()));

        userRepository.save(admin);

        log.info("Created organization {} (ID {}) with admin {}",
                organization.getOrgName(), organization.getId(), admin.getEmail());
        ...
        return RegistrationResponse.builder()
                ...
                .registrationToken(tempToken)
                .serverUrl(baseUrl)
                .message("Organization registered. Use this token in the desktop app to register your device.")
                .build();
    }

    /** Token the desktop app exchanges for an API key. Kept reversible on purpose. */
    private String generateTempToken(Long userId, Long orgId) {
        String rawToken = userId + ":" + orgId + ":" + System.currentTimeMillis();
        return cryptoService.encrypt(rawToken);
    }
