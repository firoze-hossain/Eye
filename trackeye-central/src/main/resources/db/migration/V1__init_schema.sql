...
-- V1__init_schema.sql
-- Corrected: portable syntax that works on PostgreSQL (production) and H2
-- (PostgreSQL mode). The original used MySQL-only syntax:
--   * AUTO_INCREMENT               -> GENERATED BY DEFAULT AS IDENTITY
--   * UNIQUE KEY name (col, col)   -> CONSTRAINT ... UNIQUE (...) (MySQL-only inline form)
--   * JSON column type            -> TEXT (portable; store JSON as text)
-- It also defined the audit_logs table twice, which fails on re-run.
...
CREATE TABLE IF NOT EXISTS employee_activities (
    ...
    duration_ms BIGINT NOT NULL,
    synced_at BIGINT NOT NULL,
    CONSTRAINT fk_activities_device FOREIGN KEY (device_id) REFERENCES devices(id)
    );
...
CREATE TABLE IF NOT EXISTS employee_browser_activities (
    ...
    duration_ms BIGINT NOT NULL,
    synced_at BIGINT NOT NULL,
    CONSTRAINT fk_browser_device FOREIGN KEY (device_id) REFERENCES devices(id)
    );
...
CREATE TABLE IF NOT EXISTS employee_afk_sessions (
                                                     ...
                                                     duration_ms BIGINT NOT NULL,
                                                     synced_at BIGINT NOT NULL,
                                                     CONSTRAINT fk_afk_device FOREIGN KEY (device_id) REFERENCES devices(id)
    );
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_activities_device_time ON employee_activities(device_id, start_time);
CREATE INDEX IF NOT EXISTS idx_screenshots_device_time ON employee_screenshots(device_id, timestamp);
CREATE INDEX IF NOT EXISTS idx_browser_device_time ON employee_browser_activities(device_id, start_time);
CREATE INDEX IF NOT EXISTS idx_afk_device_time ON employee_afk_sessions(device_id, start_time);
